...
| Flow | Docker | Wallet | LLM key | Notes |
|---|---|---|---|---|
| Single-agent (`npm run dev`) | No | Funded buyer for settlement | Optional with fallback | Proxy and UI only. |
| Multi-agent CoralOS round (`npm run demo:coral`) | Yes | Funded buyer/arbiter/seller env | Optional with fallback | Requires built agent images (`bash build-agents.sh`). |
