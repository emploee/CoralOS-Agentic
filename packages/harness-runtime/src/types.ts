...
export interface SellerConfig {
  name: string
  services: string[]
  /** Base business floor - a legitimate per-persona choice, never a real fetched cost. */
  floorSol: number
  persona: string
  ...
  /** The feed's /api/reputation - when set, decideBid (quote.ts) prices against the service's real
   *  recent clearing data instead of always bidding at the floor. */
