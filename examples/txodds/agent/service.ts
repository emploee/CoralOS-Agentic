 ...
 * verified TxLINE World Cup data (fixtures / odds / a de-margined "edge" read) — that's just the demo
 * that proves the rails. To build your own agent economy, return your own value here (ad copy, a
 * research brief, a routed job, a verified fact), give the seller a persona (`coral-agent.toml`), and
 * tell the buyer how to value bids. The market protocol and x402 settlement pillars don't change.
 *
 ...
 *   "edge <fixtureId>"  -> odds + fair (break-even) odds + a plain-language read     (the full loop)
 *
 * Pillars in play (all reusable for your own service):
 *   - Data     verified TxLINE fixtures/odds, fetched on devnet (TxLineClient).
 *   - Analysis turns raw data into a sellable insight, deterministically (see `agent/edge.ts`).
 *   - Solana   x402 settles the payment directly to the seller before delivery (see ../server/proxy.ts `/api/edge-x402`).
 */
...
export async function deliverService(request: string): Promise<string> {
  ...
  try {
    switch (verb) {
      ...
      // The on-thesis product: verified data in, deterministic insight out, paid in SOL.
      case 'edge': {
        const fixtureId = Number(rest[0])
